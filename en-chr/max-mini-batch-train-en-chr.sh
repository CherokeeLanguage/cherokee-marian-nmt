...
WORKDIR="/data/minibatch-work.$L1-$L2"
MODELDIR="/data/minibatch-model.$L1-$L2"
TEMPDIR="/data/minibatch-temp.$L1-$L2"
...
cd "$MODELDIR"
rm "/data/model.spm/minibatch-vocab.$L1.spm" 2> /dev/null || true
rm "/data/model.spm/minibatch-vocab.$L2.spm" 2> /dev/null || true

for x in "$MONODIR/$L2/"*".$L2"; do
    cat "$x" >> "$MONODIR/minibatch-corpus-sentencepiece.$L2"
done

sed -i 's/---//g' "$MONODIR/minibatch-corpus-sentencepiece.$L2"
sed -i '/^\s*$/d' "$MONODIR/minibatch-corpus-sentencepiece.$L2"

$MARIAN/build/spm_train --input="$MONODIR/minibatch-corpus-sentencepiece.$L2" \
    --model_prefix="/data/model.spm/minibatch-vocab.$L2.spm" \
    --vocab_size=2000 --character_coverage=1.0 --hard_vocab_limit=false

cp /dev/null "$MONODIR/minibatch-corpus-sentencepiece.$L1"
for x in "$MONODIR/$L1/"*".$L1"; do
    cat "$x" >> "$MONODIR/minibatch-corpus-sentencepiece.$L1"
done

$MARIAN/build/spm_train --input="$MONODIR/minibatch-corpus-sentencepiece.$L1" \
    --model_prefix="/data/model.spm/minibatch-vocab.$L1.spm" \
    --vocab_size=8000 --character_coverage=1.0 --hard_vocab_limit=false

mv -v "/data/model.spm/minibatch-vocab.$L1.spm.model" "/data/model.spm/minibatch-vocab.$L1.spm"
mv -v "/data/model.spm/minibatch-vocab.$L2.spm.model" "/data/model.spm/minibatch-vocab.$L2.spm"

#copy in pretrained models for SPM
cp -v /data/model.spm/minibatch-vocab* "$MODELDIR/"
...
cat $CORPUS.$L1 | /git/marian/build/spm_encode --model "$MODELDIR/minibatch-vocab.$L1.spm" > $TEMPDIR/align.temp.$L1
cat $CORPUS.$L2 | /git/marian/build/spm_encode --model "$MODELDIR/minibatch-vocab.$L2.spm" > $TEMPDIR/align.temp.$L2
...
nice $MARIAN/build/marian \
    ...
    --vocabs "$MODELDIR"/minibatch-vocab.$L1.spm "$MODELDIR"/minibatch-vocab.$L2.spm \
...
# some simple tests
    ...
    | $MARIAN/build/marian-decoder -c "$MODELDIR"/model.npz.decoder.yml --cpu-threads 16 -b 6 -n0.6 \
      --mini-batch 8 --maxi-batch 24 --maxi-batch-sort src > "$WORKDIR/fox-chicken-will-be-eating.$L2".output
